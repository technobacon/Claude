// lib/payments.js
//
// The single seam between Retype and its payment provider.
//
// HOW IT SHIPS TODAY (development mode):
//   isPaid() reads a local flag from chrome.storage. You can flip it from the
//   extension's service-worker console to test the Pro experience:
//       chrome.storage.local.set({ 'retype:paid': true })
//
// HOW TO MAKE IT REAL (one-time $19 purchase) — see NEXT_STEPS.txt (and the
// step-by-step ExtensionPay walkthrough in tabstash/NEXT_STEPS.txt, which
// applies here verbatim):
//   1. Create an account at https://extensionpay.com and register an extension.
//      Set it to a ONE-TIME payment of $19 (ExtensionPay settles via Stripe).
...
export async function openPurchaseFlow() {
  ...
  // ----------------------------------------------------------------------
  console.info(
    '[Retype] Dev build: simulate a purchase with',
    "chrome.storage.local.set({ 'retype:paid': true })",
  );
}
